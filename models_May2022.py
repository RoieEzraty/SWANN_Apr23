...
class MyModel(tf.Module):
    ...
    def __init__(self, type, num_switches, normalizer, data_shape_lst, regularizer_ODE=0, regularizer_classif=0,
        ...
        elif type == 'softmax_switch_double_notnorm':
            inputs = tf.keras.Input(shape=data_shape, name="dataset")

            if regularizer_classif == 0:
                switch = layers.Dense(num_switches, use_bias=use_bias, activation='softmax')(inputs)
            else:
                switch = layers.Dense(num_switches, use_bias=use_bias, kernel_regularizer=regularizer_classif,
                                      activation='softmax')(inputs)
            if regularizer_ODE == 0:
                x = layers.Dense(num_switches, use_bias=use_bias)(inputs)
            else:
                x = layers.Dense(num_switches, use_bias=use_bias, kernel_regularizer=regularizer_ODE)(inputs)
